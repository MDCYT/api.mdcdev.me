...
const { formatGuildRollouts } = require(join(__basedir, 'utils', 'discord', 'rollouts'));
...
const cache = new Cache("rollouts", 2, 60 * 60 * 1)
...
router.get('/', limit, async (req, res) => {
    ...
    if (!data) await axios.get('https://api.rollouts.advaith.io/').then(async response => {
