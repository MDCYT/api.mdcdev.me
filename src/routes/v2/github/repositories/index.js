...
const { betterGithubRepositoriesData, betterGithubProfileData } = require(join(__basedir, 'utils', 'github', 'utils'));
...
const repositoryLanguagesCache = new Cache("github-repositories-languages", 0, 60 * 60 * 1)
const repositoryTopicsCache = new Cache("github-repositories-topics", 0, 60 * 60 * 1)
const repositoryContributorsCache = new Cache("github-repositories-contributors", 0, 60 * 60 * 1)
const repositoryForksCache = new Cache("github-repositories-forks", 0, 60 * 60 * 1)
const repositoryStarsCache = new Cache("github-repositories-stars", 0, 60 * 60 * 1)
const repositoryWatchersCache = new Cache("github-repositories-watchers", 0, 60 * 60 * 1)
...
router.get('/:username/:repository', limit, async (req, res) => {
    ...
    if (!data) {
        await octokit.repos.get({ owner: username, repo: repository }).then(async details => {
...
router.get('/:username/:repository/languages', limit, async (req, res) => {
    ...
    if (!data) {
        await octokit.repos.listLanguages({ owner: username, repo: repository }).then(async details => {
            //If the response is 200, add the user to the cache
            if (details) {
                await repositoryLanguagesCache.set(username + "/" + repository, details);
                data = details;
            } else {
                return statusCodeHandler({ statusCode: 404 }, res);
            }
...
router.get('/:username/:repository/topics', limit, async (req, res) => {
    ...
    if (!data) {
        await octokit.repos.getAllTopics({ owner: username, repo: repository }).then(async details => {
            //If the response is 200, add the user to the cache
            if (details) {
...
router.get('/:username/:repository/contributors', limit, async (req, res) => {
    ...
    if (!data) {
        await octokit.repos.listContributors({ owner: username, repo: repository }).then(async details => {
            //If the response is 200, add the user to the cache
            if (details) {
                await repositoryContributorsCache.set(username + "/" + repository, details);
                data = details;
            } else {
                return statusCodeHandler({ statusCode: 404 }, res);
            }
...
router.get('/:username/:repository/forks', limit, async (req, res) => {
    ...
    if (!data) {
        await octokit.repos.listForks({ owner: username, repo: repository }).then(async details => {
            //If the response is 200, add the user to the cache
            if (details) {
...
router.get('/:username/:repository/stars', limit, async (req, res) => {
    ...
    if (!data) {
        await octokit.activity.listStargazersForRepo({ owner: username, repo: repository }).then(async details => {
            //If the response is 200, add the user to the cache
            if (details) {
...
router.get('/:username/:repository/watchers', limit, async (req, res) => {
    ...
    if (!data) {
        await octokit.activity.listWatchersForRepo({ owner: username, repo: repository }).then(async details => {
            //If the response is 200, add the user to the cache
            if (details) {
                await repositoryWatchersCache.set(username + "/" + repository, details);
                data = details;
            } else {
                return statusCodeHandler({ statusCode: 404 }, res);
            }
